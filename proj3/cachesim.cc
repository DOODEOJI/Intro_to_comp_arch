...
void cache_sim_t::init()
{
  ...
  used_arr = new uint64_t[sets*ways](); // automatically initialized to 0
  global_accesses = 0;
   ...
   idx_shift(rhs.idx_shift), name(rhs.name), log(false)
{
  ...
  used_arr = new uint64_t[sets*ways];
  memcpy(used_arr, rhs.used_arr, sets*ways*sizeof(uint64_t));
}
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
      used_arr[idx*ways + i] = global_accesses++;
...
uint64_t cache_sim_t::victimize(uint64_t addr) // when accessing member value, just use its name
{
  size_t idx = (addr >> idx_shift) & (sets-1); // select set
  size_t victim_index = idx * ways + 0;

  for (size_t way = 1; way < ways; way++){
    size_t index = idx * ways + way;
    if (used_arr[index] < used_arr[victim_index]) {
      victim_index = index;
    }
  }
  uint64_t victim = tags[victim_index];
  tags[victim_index] = ((addr >> idx_shift) | VALID) & ~DIRTY;
  return victim;
}
...
uint64_t* fa_cache_sim_t::check_tag(uint64_t addr)
{
  ...
  auto it = fa_tags.find(tag);
  if (it != fa_tags.end()){
    lru_list.erase(lru_map[tag]);
...
{
  uint64_t old_tag = 0;
  if (fa_tags.size() == ways) // select victim
  {
    uint64_t victim_tag = lru_list.back();
    old_tag = fa_tags[victim_tag];

    lru_list.pop_back();
    fa_tags.erase(victim_tag);
    lru_map.erase(victim_tag);
  }
  uint64_t tag = addr >> idx_shift;
  fa_tags[tag] = (tag | VALID) & ~DIRTY;
